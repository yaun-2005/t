...
def start(message):
    ...
    bot.send_message(
        ...
        "<tg-emoji emoji-id='5251299553239398548'>🤖</tg-emoji> <b>မင်းတို့ကြမ်းတာထုပ်ထားတယ်             𝐏ᴏᴡᴇʀ 𝐁ʏ✿︎ -  @LeyoxYan_Yan !</b>\n\n"
...
# --- Auto Register Group Section ---
    ...
    if message.chat.type == "private" and user_id not in ADMIN_IDS:
        if not is_joined(user_id):
            ...
            join_kb.add(InlineKeyboardButton(" Join (စစ်ဆေးမည်)", url=f"https://t.me/NoLoveMusicUpate?start=start"))
            return bot.send_message(
                message.chat.id,
